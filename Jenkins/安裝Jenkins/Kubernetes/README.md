# Kubernetes上安裝 Jenkins 安裝教學
## 前提
1. 去安裝jenkins安裝包 (https://artifacthub.io/packages/helm/jenkinsci/jenkins?modal=install)
2. 要有Helm 
3. 已有Cloudflare tunnel
4. 有k3s或k8s都可以

## 開始安裝Jenkins
1. 進入資料夾cd /home/primary-drive/DevOps/Jenkins
2. ls 確認是否有這兩個檔 
```
qi@omvnas:/home/primary-drive/DevOps/Jenkins$ ls
jenkins-admin-secret.yaml  values.yaml
```
3. 創建命名空間jenkins
```
 kubectl create namespace jenkins
```
4. jenkins-admin-secret.yaml
```
apiVersion: v1
kind: Secret
metadata:
  name: jenkins-admin-secret  # Secret 名稱
  namespace: jenkins  # 確保與 Jenkins 部署的 namespace 相同
type: Opaque
data:
...
qi@omvnas:/home/primary-drive/DevOps/Jenkins$ kubectl get secret -n jenkins
NAME                   TYPE     DATA   AGE
jenkins-admin-secret   Opaque   2      5s
...
helm repo update
```
7. 確認k3s 有local-path 修改values.yaml 需要(其他名字就要記下來) 
```
kubectl get storageclass
```
```
qi@omvnas:/home/primary-drive/DevOps/Jenkins$ kubectl get storageclass
...
```
 ...
 4. persistence:
        ...
        enabled: true
        size: 300Gi                        (依照自己電腦硬碟配置,我NAS所以配置300G, 一般電腦1T配置50G就好)
        ...
        storageClass: "local-path"         (剛剛教你記住的地方)
...
```
```
qi@omvnas:/home/primary-drive/DevOps/Jenkins$ kubectl get pods -n jenkins
NAME        READY   STATUS    RESTARTS   AGE
jenkins-0   2/2     Running   0          39m
qi@omvnas:/home/primary-drive/DevOps/Jenkins$ kubectl get svc -n jenkins
NAME            TYPE        CLUSTER-IP    EXTERNAL-IP   PORT(S)     AGE
jenkins         ClusterIP   10.43.32.65   <none>        8080/TCP    13h
jenkins-agent   ClusterIP   10.43.55.1    <none>        50000/TCP   13h
qi@omvnas:/home/primary-drive/DevOps/Jenkins$ kubectl get pv
NAME                                       CAPACITY   ACCESS MODES   RECLAIM POLICY   STATUS   CLAIM                                                                                                                      STORAGECLASS   VOLUMEATTRIBUTESCLASS   REASON   AGE
pvc-2af26451-1bc7-43f1-a02c-099b6c6dc8fe   200Gi      RWO            Delete           Bound    kubegems-monitoring/prometheus-kube-prometheus-stack-prometheus-db-prometheus-kube-prometheus-stack-prometheus-0           local-path     <unset>                          12d
pvc-3589a860-6f1a-4433-8904-5da0742a1b3e   8Gi        RWO            Delete           Bound    kubegems/data-kubegems-mysql-0                                                                                             local-path     <unset>                          12d
pvc-5ec977bf-674b-4b9a-85b7-e6caafdc5f7a   10Gi       RWO            Delete           Bound    kubegems/data-kubegems-gitea-0                                                                                             local-path     <unset>                          12d
pvc-696125e1-c290-4f41-909b-89fbf60ba51f   8Gi        RWO            Delete           Bound    kubegems/redis-data-kubegems-redis-master-0                                                                                local-path     <unset>                          12d
pvc-6e8c419d-2c52-4382-bcd7-7645650b256b   10Gi       RWO            Delete           Bound    kubegems-monitoring/alertmanager-kube-prometheus-stack-alertmanager-db-alertmanager-kube-prometheus-stack-alertmanager-0   local-path     <unset>                          12d
pvc-be17278f-cf28-4e12-b242-005df0123866   300Gi      RWO            Delete           Bound    jenkins/jenkins                                                                                                            local-path     <unset>                          32m

```
10. cloudflare tunnel 配置
```
jenkins.totosss0527.com> http://10.43.32.65:8080
```
11. 進入畫面後安裝套件
```
Matrix Authorization Strategy (管理用戶)
```
